...
```{r}
computos_tbl <-
  read_csv("votacion_simulada_simulacro1_OK.csv")  |>
...
n_rep <- 7
set.seed(1488810)
p_obs_seq <- seq(0.05, 0.95, 0.10)
muestra <- select_sample_prop(computos_tbl, stratum = ID_ESTRATO, frac = 0.03063)
muestras <- map_dfr(1:length(p_obs_seq),  function(n_rep){
   muestra <- slice_sample(muestra, prop = p_obs_seq[n_rep])
   muestra$num_muestra <- n_rep
   muestra
})
```
...
resultados_lst <- parallel::mclapply(seq_along(p_obs_seq), function(n_rep){
  ...
  res <- quickcountmx::hb_estimation(muestra,
    ...
    num_warmup = 150, num_iter = 80, chains = 8, 
    model = "consulta", nominal_max = 3050, part = TRUE, prop_obs = 0.9 * p_obs_seq[n_rep]) |> 
    pluck(1)
...
```{r}
write_rds(resultados_tbl, "resultados-nbh-6.rds")
#resultados_tbl <- read_rds("resultados-nbh-5.rds")
...
resultados_r_tbl <- map_dfr(seq_along(p_obs_seq), function(num){
...
ggplot(res_union_tbl |> left_join(resultados_finales), 
  ...
  geom_linerange() +
  facet_grid(party ~ metodo, scales = "free") + 
  geom_hline(data = resultados_finales, aes(yintercept = pct_obs/100), colour = "red")  # nolint
```


```{r}
ggplot(res_union_tbl |> filter(num_muestra > 6) |> left_join(resultados_finales),
       aes(x = num_muestra, ymin = inf, ymax = sup)) +
  geom_linerange() +
  facet_grid(party ~ metodo, scales = "free") + 
  geom_hline(data = resultados_finales, aes(yintercept = pct_obs/100), colour = "red")  # nolint
